...
#        f.close()
    ...
    if HisGrid.get()=="":
        ...
        logbook.QsoForm(mcall,mgrid,operator,qso_date,qso_time,his_grid,rpt_sent,rpt_rcvd,\
                        pwr,tf,sm,last_update,band,mode)
...
filemenu.add('command', label = 'Open', command = openfile,accelerator='Ctrl+O')
filemenu.add('command', label = 'Open next in directory', command = opennext,accelerator='F6')
filemenu.add('command', label = 'Decode remaining files in directory',command = decodeall, accelerator='Shift+F6')
filemenu.add_separator()
filemenu.add('command', label = 'Delete all *.WAV files in RxWav',command = delwav)
filemenu.add_separator()
...
root.bind_all('<Alt-Z>',toggle_zap)
root.bind_all('<Shift-h>',logbook.QsoFormHelp) # Log Form Help
root.bind_all('<Shift-H>',logbook.QsoFormHelp)
