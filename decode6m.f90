subroutine decode6m(data,jz,cfile6,MinSigdB,istart,NFixLen,lcum,     &
     f0,lumsg,npkept,yellow)
...
! Is it time to write out the results?
        if((n.eq.48) .or. (j.ne.j0+3 .and. j0.ne.0)) then
           ...
           if(nsig.ge.nslim) then
              ...
              if(lcum) write(21,1010) cfile6,tping,width,nsig,ndf0,    &
                   (msg(k:k),k=1,n)
