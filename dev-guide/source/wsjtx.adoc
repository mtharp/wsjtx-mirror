...
Several targets exist for **_WSJT-X_**, both in Debug and Release. The commands
outlined below are used to checkout and build the Development Branch for types. 
...
Only after successfully building *Hamlib3* should you proceed to the
**_WSJT-X_** builds menus.
****

To build **Hamlib3**, simply select it form the Main Menu. If the build was
successful, you will be presented with the <<HAMLIB3SUCESS,image>> below. You 
...
[[HAMLIB3SUCESS]]
.Successful Hamlib3 Build Summary
...
Several targets exist for **_WSJT-X_**, both in Debug and Release. The selections
...
|Debug Target
|Default install target build with Debugin enabled
...
|Help
|Explanation for each build option
...
various elements relevant to that particular build. A similar summary will be 
presented for all *JTSDK-Nix* builds that use the menu system.

.Successful WSJT-X Build Summary
