...
! In each time slice, find largest peak between LTone*NBaud-DFTolerance and
! (LTone+3)*NBaud+DFTolerance.
...
! constant should be dB(43/2500) = -17.6 dB?

     if(peak.gt.stlim) then
! To minimize false ST decodings from QRN and MT pings, find the
! second best peak (excluding points around the first peak).
        smax2=0.
        do i=ia,ib
...
! Larger values of ratlim make it more likely to report ST decodings.
        ratlim=0.18
...
! Now check for redundant detections.  (Not sure, now, why I chose
! the time span 0.11 s.)
...
! OK, we want to output this one.  Save the information.
