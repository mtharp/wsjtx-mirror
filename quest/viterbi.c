...
#ifdef V217
#define	K 17			/* Constraint length */
#define N 2			/* Number of symbols per data bit */
#define Polys	Poly217		/* Select polynomials here */
#endif

#ifdef V218
#define	K 18			/* Constraint length */
#define N 2			/* Number of symbols per data bit */
#define Polys	Poly218		/* Select polynomials here */
#endif

#ifdef V314
#define	K 14			/* Constraint length */
#define N 3			/* Number of symbols per data bit */
#define Polys	Poly314		/* Select polynomials here */
#endif

#ifdef V315
#define	K 15			/* Constraint length */
#define N 3			/* Number of symbols per data bit */
#define Polys	Poly315		/* Select polynomials here */
#endif

#ifdef V316
#define	K 16			/* Constraint length */
#define N 3			/* Number of symbols per data bit */
#define Polys	Poly316		/* Select polynomials here */
#endif

#ifdef V413
#define	K 13			/* Constraint length */
#define N 4			/* Number of symbols per data bit */
#define Polys	Poly413		/* Select polynomials here */
#endif
...
#define	K 15			/* Constraint length */
#define N 4			/* Number of symbols per data bit */
#define Polys	Poly415		/* Select polynomials here */
#endif
...
#ifdef V813
#define	K 13			/* Constraint length */
#define N 8			/* Number of symbols per data bit */
#define Polys	Poly813		/* Select polynomials here */
#endif
...
unsigned int Poly217[] = {0347241, 0246277};	/* k = 17  */
unsigned int Poly218[] = {0506477, 0673711};	/* k = 18  */
...
unsigned int Poly315[] = {067175,052137,056463};     /* k = 15 */
unsigned int Poly316[] = {0110763,0136467,0154535};  /* k = 16 */
...
unsigned int Poly415[] = {067651,047115,053613,075663}; // k = 15
...
int *nn)                        // N
{
  ...
  encode(symbols,data,nbytes,0,0); // Do the encoding
...
long *Metric              /* Final path metric (bigger is better) */
  ...
  long metric;
  viterbi(&metric,ddec,symbols,*Nbits,mettab,0,0);
  *Metric=metric;
}
...
//    printf("B  %d  %d  %d\n",bitcnt+1,endstate,nmetric[endstate]);
    if(++bitcnt == nbits){
  ...
  for(i=nbits-1;i >= 0;i--){
    ...
    //    if(i>65) printf("B  %d  %d  %d  %d  %d  %d  %d\n",i,endstate,(1 << (K-1)),m0,m1,pp[m0],ipp);
    if(pp[m0] & m1) {
      endstate |= (1 << (K-1));
      data[i>>3] |= 0x80 >> (i&7);
    }
