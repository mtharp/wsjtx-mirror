...
# Date            : 2014
# Version         : 0.1.0
# Usage           : ./data-uri.sh [ option ]
# Notes           : Requires: Python 2.7+, AsciiDoc
# Copyright       : GPLv(3)
#
...
WSPR="$BASEDIR/wspr"
WSPRX="$BASEDIR/wsprx"
TOC="asciidoc.py -b xhtml11 -a data-uri -a toc2 -a iconsdir=icons -a max-width=1024px"
declare -a all_apps_ary=( 'map65' 'wsjt' 'wsjtx' 'wspr' 'wsprx' )
declare -a short_list=( 'map65' 'wsjt' 'wspr' 'wsprx')
...
function clean_exit() {
	...
	for i in "${all_apps_ary[@]}"
		do
			cd "$BASEDIR"
			TMP="$BASEDIR/$i/tmp"
      IMG="$BASEDIR/$i/source/images"
      ICO="$BASEDIR/$i/source/icons"
...
function build_all_guides() {
	clear
	pre_file_check
	echo -e ${C_Y}"Building All DATA-URI Documentation"${C_NC}
	echo
...
do
	...
	[Yy]* )
		...
		# Short List Loop: map65 wsjt wspr wsprx
		for f in ${short_list[@]}
		do
			app_name="$f"
			dir_=$(echo "$f" | tr [:upper:] [:lower:])
			cd "$dir_"
    ...
    remove_image_folders
		...
		echo
		echo -e ${C_Y}'Finished Building DATA-URI Documentation'${C_NC}
    echo
		return
	;;
	[Nn]* )
...
mkdir $BASEDIR/$app_name/source/icons
rsync -aq --exclude=.svn $BASEDIR/icons/ $BASEDIR/$app_name/source/icons

mkdir $BASEDIR/$app_name/source/images
...
rm -r $BASEDIR/$app_name/source/icons
...
function app_menu_help() {
	clear
	echo -e ${C_G}"WSJT DATA-URI HELP MENU\n"${C_NC}
	echo 'USAGE: data-uri.sh [ option ]'
	echo
	echo 'OPTION: All map65 wsjt wsjtx wspr wsprx help'
	echo
	echo 'Example(s):'
	echo 'Build All Guides:  ./data-uri.sh all'
	echo 'Build WSJT-X Only: ./data-uri.sh wsjtx'
	echo
