...
!       1. real transform for the 1st dimension, n even.  method--
...
!    shuffle data by bit reversal, since n=2**k.
...
!    w=exp(-2*pi*sqrt(-1)*m/(4*mmax)).  check for w=-sqrt(-1)
!    and repeat for w=w*(1-sqrt(-1))/sqrt(2).
...
! Complete a real transform in the 1st dimension, n even, by conjugate \
! symmetries.
